...
def show_grid_images(images, label, height=8, width=8):
...
def dataloader_to_images(dataloader):
    images, labels = [], []
...
def show_images_in_clusters(images, predicted_clusters):
    num_clusters = np.unique(predicted_clusters)
    for num in num_clusters:
        print('cluster {} has {} images'.format(num, np.sum(predicted_clusters == num)))
...
class Clustering():
    def __init__(self, hparams):
        ...
        self.feature_ext.load_from_checkpoint(checkpoint_path=self.path_features_ext)
