...
def load_mlflow(output: str) -> dict:
...
def write_mlflow(output: str):
    ...
    mlflow_run_id = (
        mlflow_run.info.run_id if mlflow_run is not None else "NO_MLFLOW"
    )
...
def train_impl(
    ...
    ----------
    model : object
        The neural network model object.
    datamodule : object
        lightning data module that will instantiate data loaders.
    output : str
        Output directory.
    hyper_params : dict
        Dict containing hyper-parameters.
    use_progress_bar : bool
        Use tqdm progress bar (can be disabled when logging).
    start_from_scratch : bool
    ...
    resume_from_checkpoint = handle_previous_models(
        output, last_model_path, best_model_path, start_from_scratch
    )
...
def handle_previous_models(
    ...
    if len(last_models + best_models) > 0:
        ...
        last_models = glob.glob(
            new_folder + os.path.sep + LAST_MODEL_NAME + os.sep + "*"
        )
